...
static int update_ns_elem(ns_track_elem_t *ns_elem, ns_seg_info_t *info)
{
    ...
    //fprintf(stderr, "update_ns_elem   tmp %x ns_elem->num_meta_seg %lu, info->num_meta_seg %lu\n", tmp, ns_elem->num_meta_seg, info->num_meta_seg);
    for (i = ns_elem->num_meta_seg; i < info->num_meta_seg; i++) {
        if (0 == is_client) {
            fprintf(stderr, "create_new_segment META i = %d\n", i);
            seg = create_new_segment(NS_META_SEGMENT, info->ns_name, i);
        } else {
            fprintf(stderr, "attach_new_segment META i = %d\n", i);
...
static ns_track_elem_t *add_new_namespace(char *nspace)
{
    ...
    PMIX_LIST_FOREACH(new_elem, &namespace_info_list, ns_track_elem_t) {
        if (0 == strncmp(nspace, new_elem->ns_name, PMIX_MAX_NSLEN+1)) {
            ...
            fprintf(stderr, "<server> add_new_namespace ns tracked\n");
            return new_elem;
        }
    }
    fprintf(stderr, "<server> add_new_namespace create elem\n");
    ...
    if (PMIX_SUCCESS != rc) {
        ...
        fprintf(stderr, "<server>update_ns_elem failed\n");
        return NULL;
    }
//    fprintf(stderr, "     <server>   add to namespace_info_list ns %s pid = %d\n", nspace, getpid());
...
{
    fprintf(stderr, " update_rank_meta_info \n");
    ...
    if (NUM_META_ELEMS > num_elems) {
        fprintf(stderr, "update_rank_meta_info NUM_META_ELEMS %d > num_elems %d metaseg %x\n", NUM_META_ELEMS, num_elems, metaseg);
    } else {
        fprintf(stderr, "update_rank_meta_info extend meta segment for ns %s\n", ns_info->ns_name);
        /* extend meta segment, so create a new one */
        tmp = extend_segment(tmp, ns_info->ns_name);
        if (NULL == tmp) {
            fprintf(stderr, "update_rank_meta_info extend_segment failed\n");
            return PMIX_ERROR;
        }
...
static size_t put_data_to_the_end(ns_track_elem_t *ns_info, seg_desc_t *dataseg, char *key, void *buffer, size_t size, int flag)
{
    ...
    fprintf(stderr, "  <server> put key %s at rel offset %lu till = %lu abs shift = %lu size %lu\n", key, offset, data_ended, id * NS_DATA_SEG_SIZE, size);
...
static int pmix_sm_store(ns_track_elem_t *ns_info, int rank, pmix_kval_t *kval, rank_meta_info **rinfo, int replace_flag)
{
    ...
    if (0 == replace_flag) {
        ...
        (*rinfo)->count++;
        fprintf(stderr, "  <server> put data finished for rank %d offset %lu count %lu\n\n", (*rinfo)->rank, (*rinfo)->offset, (*rinfo)->count);
    } else if (NULL != *rinfo) {
        ...
        if (NULL == addr) {
            ...
            fprintf(stderr, "replace addr is unknown\n");
        }
...
//        size_t kval_cnt = (*rinfo)->count + 1;
    //size_t ofs= (*rinfo)->offset;
        ...
        while(1) {
            ...
            if (0 == strncmp(addr, "EXTENSION_SLOT", PMIX_MAX_KEYLEN)) {
                ...
                if (0 < offset) {
        //            fprintf(stderr, " replace  EXTENSION_SLOT is busy with %lu ofs = %lu\n", offset, ofs);
       ...
       //             fprintf(stderr, " replace   EXTENSION_SLOT should be updated ofs = %lu\n", ofs);
                    if (0 == update_ext_slot) {
                        /* add to the end */
                        offset = put_data_to_the_end(ns_info, datadesc, kval->key, buffer->base_ptr, size, 0);
       //                 fprintf(stderr, "replace item not found ext slot empty, put key %s to the end. stored in %lu\n", kval->key, offset);
                        size_t tmp = 0.0;
                        put_data_to_the_end(ns_info, datadesc, "EXTENSION_SLOT", (void*)&tmp, sizeof(size_t), 1);
      //                  fprintf(stderr, " store key %s offset %lu\n", kval->key, offset);
                        (*rinfo)->count++;
    ...
    //            fprintf(stderr, "replace target key found key %s ofs = %lu\n", kval->key, ofs);
                /* target key is found, compare value sizes */
                size_t cur_size = *(size_t *)(addr + PMIX_MAX_KEYLEN);
                if (cur_size != size) {
                //if (1) {
    //                fprintf(stderr, "replace mark invalidated\n");
    ...
    //                fprintf(stderr, "replace in place ofs = %lu\n", ofs);
    ...
    //            fprintf(stderr, "replace skip ofs = %lu\n", ofs);
...
fprintf(stderr, "replace_flag = %d key %s\n", replace_flag, kp->key);
    ...
    fprintf(stderr, "<server>update rank meta info\n");
...
int sm_data_store(char *nspace, int rank, pmix_buffer_t *buf)
{
    ...
    fprintf(stderr, "<server> add_new_namespace %s\n", nspace);
...
fprintf(stderr, "<server> store_data_for_rank %s:%d\n", nspace, rank);
...
static ns_track_elem_t *add_new_namespace_by_client(ns_seg_info_t *ns_info, char *nspace)
{
    ...
    rc = update_ns_elem(new_elem, ns_info);
    strncpy(new_elem->ns_name, nspace, PMIX_MAX_NSLEN);
//    fprintf(stderr, "<client> add_new_namespace_by_client new-elem->metaseg = %x\n", new_elem->meta_seg);
    if (PMIX_SUCCESS != rc) {
        /* print */
        PMIX_RELEASE(new_elem);
        return NULL;
    }
    //fprintf(stderr, "     <client>   add to namespace_info_list ns %s pid = %d\n", nspace, getpid());
    pmix_list_append(&namespace_info_list, &new_elem->super);
...
int sm_data_fetch(char *nspace, int rank, char *key, pmix_value_t **kvs)
{
    ...
    fprintf(stderr, "<client>update_initial_segment_info rank = %d\n", rank);
    update_initial_segment_info();

    elem = get_ns_info_from_initial_segment(nspace);
    fprintf(stderr, "<client>get_ns_info_from_initial_segment %x rank %d\n", elem, rank);
    if (NULL == elem) {
        /* no data for this namespace is found in the shared memory. */
        return PMIX_ERROR;
    }

    fprintf(stderr, "<client>add_new_namespace_by_client ns_info->num_meta_seg = %lu name = %s rank %d\n", elem->num_meta_seg, elem->ns_name, rank);
    ...
    fprintf(stderr, "<client> added new ns by client rank %d\n", rank);
    ...
    if (0 == count) {
        ...
        fprintf(stderr, "<client> !!!! no data for this rank is found in the shared memory. rank %d\n", rank);
        return PMIX_ERROR;
    } else {
    ...
    if (NULL == rinfo) {
        fprintf(stderr, "<client>no data for this rank %d\n", rank);
        /* no data for this rank is found in the shared memory. */
        return PMIX_ERROR;
    }
    unsigned char *addr;
    ...
    //fprintf(stderr, "<client> data region addr %x\n", addr);
    ...
    //        fprintf(stderr, "<client> data fetch   -invalidated rank %d\n", rank);
            /*skip it */
            size_t size = *(size_t *)(addr + PMIX_MAX_KEYLEN);
            addr += PMIX_MAX_KEYLEN + sizeof(size_t) + size;
   //         ofs += PMIX_MAX_KEYLEN + sizeof(size_t) + size;
        } else if (0 == strncmp(addr, "EXTENSION_SLOT", PMIX_MAX_KEYLEN)) {
   //         fprintf(stderr, "<client> data fetch   -ext slot rank %d ofs = %lu\n", rank, ofs);
   ...
   //             fprintf(stderr, "<><><><>><<><<><>><><>><>><>><> no more data for this rank %d key %s\n", rank, key);
                break;
            }
        } else if (0 == strncmp(addr, key, PMIX_MAX_KEYLEN)) {
   //         fprintf(stderr, "<client> data fetch   -target key %s rank %d ofs = %lu\n", key, rank, ofs);
            ...
            *kvs = &val;
            ...
            size_t size = *(size_t *)(addr + PMIX_MAX_KEYLEN);
    //        fprintf(stderr, "<client> data fetch   -skip key %s, look for %s rank %d size %lu offset %lu rinfo->offset %lu ofs = %lu\n", ckey, key, rank, size, PMIX_MAX_KEYLEN + sizeof(size_t) + size, rinfo->offset, ofs);
            addr += PMIX_MAX_KEYLEN + sizeof(size_t) + size;
     //       ofs += PMIX_MAX_KEYLEN + sizeof(size_t) + size;
        }
        rinfo->count++;
